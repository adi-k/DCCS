...
const newLine = () => {
	...
	if (_counter<9) {
	var _newLine= '<br> <p id="paragraphAdmin'+_counter+'" class="searchField">Element '
	+ _counter 
	...
	+'id="selectFirst'
	+_counter
	+'" name="componentList" onclick="updateRadio('+_counter+')">'
    +'<option value="text" onclick="updateRadioClear('+_counter+')">Textbox</option>'
    +'<option value="checkbox" onclick="updateRadioClear('+_counter+')">Check Box</option>'
	...
	+'</select>'
    +'<pre class="searchField">   </pre>'
	+'<input type="button" value="+" class="plusButton" id="plusBtn'+_counter+'"  onclick="newLine()">';
...
const adminPageNewForm = () => {
	...
	//adding new elements, this whole little web app reminds me of MS ACCESS when creating layouts
	...
	var _saveResetInsert='<input type="button" value="SAVE" id="saveButton" onclick="saveNewForm()"><input type="button" value="CLEAR" id="clearButton" onclick="clearForm()">';
...
const saveNewForm = () => {
	...
	if (localStorage.getItem(_formName) === null) {
		...
		if(confirm("A form with this name already exists, pls choose another name or press YES to overwrite current form (!)")){
		_formName=_formName.replace("(example)","")
		localStorage.setItem(_formName, _tamperedForm);
		}
		else{
			_formName = prompt("Please enter a name for your new form (Be aware if you enter the same way the form template WILL be overwritten!", "Janes form (example)");
			_formName=_formName.replace("(example)","")
			localStorage.setItem(_formName, _tamperedForm);
...
const additionalDataSaveNewForm = (_formName) => {
	...
	for (var i = 1; i < howManyElements()+1; i++) {
		...
		_dropdownSelected=_dropdownSelected.options[ _dropdownSelected.selectedIndex ].value ;
		_savedData.push(_dropdownSelected);	
		...
		if(_dropdownSelected=="radio"){
			...
			if(x!=null){_radioObj[_objHelper]+=x.value;}
...
*/
	...
	do {
		...
		_insert+='<p class="searchField">'+_drawHelper[i]+'</p>';
		i+=1;


		switch(_drawHelper[i]) {
			    case "text":
			    _insert+='<input type="text" class="searchField" value=""><br>';
			        break;
			    case "checkbox":
			    _insert+='<input type="checkbox" class="searchField" value=""><br>';
			        break;
			    case "radio":
		...
		if (_drawHelper[i]=="mandatory") {}
		else if (_drawHelper[i]=="nonmandatory"){}
		i+=1;
...
console.log(_drawHelperRadio);
console.log(_drawHelper);
...
const drawRadio = (_loopCounter, _drawHelperRadio) => {
	...
	var x=_helper[0];
	if(x!=null){_insert+='<input type="radio" name="" class="searchFieldFormRadio" value="'+_helper[0]+'">'+_helper[0]+'<br>';};

	var x=_helper[1];
	if(x!=null){_insert+='<input type="radio" name="" class="searchFieldFormRadio" value="'+_helper[1]+'">'+_helper[1]+'<br>';};

	var x=_helper[2];
	if(x!=null){_insert+='<input type="radio" name="" class="searchFieldFormRadio" value="'+_helper[2]+'">'+_helper[2]+'<br>';};
...
const updateRadio = (_elem) => {
	...
	_newLine='<select class="chooseRadio" id="radioChosen'+_tempElem+'" onclick="updateRadioQuant('+_tempElem+')">'
	...
	if(x!=null){document.getElementById("radioChosen"+_tempElem).outerHTML="";}

	_elem = document.getElementById(_elem);
	_elem.insertAdjacentHTML('afterend', _newLine);
	}
	else{updateRadioClear(_tempElem);} //if the radio option is deselected, remove it
...
const updateRadioClear = (_tempElem) => {
	...
	var x=document.getElementById("radioChosen"+_tempElem+"_radioChosenLabel1");
	if(x!=null){document.getElementById("radioChosen"+_tempElem+"_radioChosenLabel1").outerHTML="";}

	x=document.getElementById("radioChosen"+_tempElem+"_radioChosenLabel2");
	if(x!=null){document.getElementById("radioChosen"+_tempElem+"_radioChosenLabel2").outerHTML="";}


    x=document.getElementById("radioChosen"+_tempElem+"_radioChosenLabel3");
	if(x!=null){document.getElementById("radioChosen"+_tempElem+"_radioChosenLabel3").outerHTML="";}
...
const updateRadioQuant = (_tempElem) => {
	//update the number of radio options, if one changes his mind to expand or decrease
	    	...
	    	switch(_dropdownSelected) {
					    case "1":
					        _insert='<input type="text" class="radioLabels" id="radioChosen'+_tempElem+'_radioChosenLabel1" name="" value="Radio Label 1" >';
					        break;
					    case "2":
					        _insert='<input type="text" class="radioLabels" id="radioChosen'+_tempElem+'_radioChosenLabel1" name="" value="Radio Label 1" >'
					        +'<input type="text" class="radioLabels"  id="radioChosen'+_tempElem+'_radioChosenLabel2" name="" value="Radio Label 2" >';
					        break;
					    case "3":
					        _insert='<input type="text" class="radioLabels"  id="radioChosen'+_tempElem+'_radioChosenLabel1" name="" value="Radio Label 1" >'
					        +'<input type="text" class="radioLabels"  id="radioChosen'+_tempElem+'_radioChosenLabel2" name="" value="Radio Label 2" >'
					        +'<input type="text" class="radioLabels"  id="radioChosen'+_tempElem+'_radioChosenLabel3" name="" value="Radio Label 3" >';
					        break;
...
const selectedIndexStay = () => {
	...
	for (var i = 1; i <9 ; i++) {
		...
		_tempElem=document.getElementById('radioChosen'+i+'_radioChosenLabel1');
		if(_tempElem!=null){document.getElementById("selectFirst"+i).selectedIndex = "2";};
	}
...
const updateFormList = () => {
	...
	_tempBool2=_keyForm.includes(_substring2);
  	 ...
  	 if (!_tempBool1 && !_tempBool2) {
  	 	  	document.getElementById("dropdownForms").innerHTML += '<option value="'+_keyForm+'">'+ _keyForm+'</option>';
  	 }
